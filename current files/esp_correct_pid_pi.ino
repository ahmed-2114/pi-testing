...
//
// Pi bridge note:
//   This file is copied from correct_pid.ino and keeps the same controller,
//   gains, signs, motor mapping, GUI, and RTOS control loop. The only added
//   layer is a UART JSON bridge for pi_master.py / pi_mecanum_path_builder.py.
//
...
float positionToleranceCm = 1.0f;
float yawToleranceDeg = 3.0f;
bool imuOk = false;
...
float displayedOdomYawDeg() {
  return imuOk ? displayedImuYawDeg() : encoderYawDeg();
}
...
float runYawPositionController(float error, float outputLimitRpm) {
  ...
  const float yawRateDps = imuOk ? displayedImuGyroDpsFilt() : 0.0f;
  const float absError = fabsf(error);
  const float absRate = fabsf(yawRateDps);

  state.pTerm = yawKp * error;
  state.iTerm = 0.0f;

  float dampingRpm = imuOk ? (-YAW_DAMP_RPM_PER_DPS * yawRateDps) : 0.0f;
...
void updatePositionController(float dt) {
  ...
  bool yawRateDone = !imuOk || fabsf(displayedImuGyroDpsFilt()) <= YAW_SETTLE_RATE_DPS;
...
String buildPiTelemetryJson() {
  ...
  s += ",\"rawYawDeg\":" + String(imuYawDeg, 3);
...
void startPiPositionMove(uint32_t seq, float angleDeg, float distanceCm, float headingDeg) {
  ...
  // pi_master.py uses angle -90 for right. In the working GUI controller,
  // positive targetStrafeCm is the rightward direction with strafeSign=-1.
...
<body>
  ...
  <script>
    ...
    async function refresh() {
      ...
      field('headingSource').textContent = data.imu.headingUsesImu ? 'IMU GYRO' : 'ENCODER';
...
void setup() {
  ...
  Serial.println();
  Serial.println("Clean mecanum position controller started.");
  ...
  xTaskCreatePinnedToCore(uartTask, "pi-uart", 6144, nullptr, 3, &uartTaskHandle, 0);
  xTaskCreatePinnedToCore(piTelemetryTask, "pi-telemetry", 6144, nullptr, 1, &piTelemetryTaskHandle, 0);

  sendJsonLine("{\"type\":\"ready\",\"baud\":115200,\"source\":\"correct_pid_pi\",\"imu\":" + String(imuOk ? "true" : "false") + "}");
}
